...
def detect_circles(
  ...
  detected_circles = cv2.HoughCircles(blur, cv2.HOUGH_GRADIENT, 1, min_dist, param1 = param_1, param2 = param_2, minRadius = min_radius, maxRadius = max_radius)

  if detected_circles is None:
    raise ConnectionAbortedError("No circles detected.")
  
  mean_radius = np.rint(np.average(detected_circles[0], axis=0)*radius_percent)[2].astype(int)
  ...
  if sorting_type == 'h':
    detected_circles = np.uint16(sorted(detected_circles , key=lambda k: [k[1], k[0]]))

  if sorting_type == 'v':  
    detected_circles = np.uint16(sorted(detected_circles , key=lambda k: [k[0], k[1]]))

  detected_circles = np.hstack((detected_circles, np.full((detected_circles.shape[0], 1), mean_radius)))
...
def draw_circles(img: Image, pts: np.array, numered=True):
  ...
  for i, pt in enumerate(pts):
    cv2.circle(image_copy, (pt[0], pt[1]), pt[2], (0, 0, 255), int(np.ceil(np.prod(img.dim)*1e-7)))

    cv2.circle(image_copy, (pt[0], pt[1]), 1, (255, 0, 0), int(np.ceil(np.prod(img.dim)*1e-6)))

    if(numered):
      cv2.putText(image_copy, str(i+1), (pt[0]-10, pt[1]+15), cv2.FONT_HERSHEY_SIMPLEX, int(np.ceil(np.prod(img.dim)*4e-7)), (0, 0, 0), int(np.ceil(np.prod(img.dim)*5e-7)), cv2.LINE_AA)
...
def av_value(pts: np.array, image: Image, mode = 0):
  ...
  for pt in pts:
    ...
    if mode == 0:
      rgb_value = np.round(cv2.mean(spot_image.image, mask=masked_region)[:-1]).astype(int)
...
def find_av_with_half_height(hists: np.array, height_calc = 0.5):
  ...
  for i in range(len(hists)):
    ...
    left_index = np.where(hist[:max_index] <= half_height)[0][-1] if np.any(hist[:max_index] <= half_height) else 0
    right_index = np.where(hist[max_index:] <= half_height)[0][0] + max_index if np.any(hist[max_index:] <= half_height) else 255
...
def hsv_thresh(img: cv2.Mat, mask=None):
  ...
  h_left_index, h_right_index = np.clip(h_max - 20, 0, 180), np.clip(h_max + 20, 0, 180)

  s_max_value = hist_s[s_max].astype(int)[0]
  half_height = s_max_value * 0.1
  s_left_index = np.where(hist_s[:s_max] <= half_height)[0][-1] - 5 if np.any(hist_s[:s_max] <= 20) else 0
  s_right_index = np.where(hist_s[s_max:] <= 20)[0][0] + s_max + 5 if np.any(np.where(hist_s[s_max:] <= 20)) else 255

  s_left_index, s_right_index = np.clip(s_left_index, 0, 255), np.clip(s_right_index, 0, 255),
...
def cut_spot(pt, img):
  spot_image = img.image[pt[1]-(pt[2]+10):pt[1]+(pt[2]+10), pt[0]-(pt[2]+10):pt[0]+(pt[2]+10)]
